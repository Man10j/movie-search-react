...
const Card = (props) => {
      ...
      return (
        <div key={movie.id} className="movie_list_card">
          {movie.poster_path ? (
            ...
            <img src={Noimg} className="noimg" alt="movieposter" />
          )}
  ...
  return <div className="card_list">{values}</div>;
};
...
const mapDispatchToProps = (dispatch) => {
  return {
    updateWishlist: (wishlistmovie) => {
      dispatch({ type: "UPDATE_WISHLIST", payload: wishlistmovie });
    },
  };
};
